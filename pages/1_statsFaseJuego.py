...
if df.empty:
    ...
    resumen_defensas['% Exito'] = (resumen_defensas['Goles']  / resumen_defensas['Defensas']) * 100

    todas_fases_defensa = pd.DataFrame([item['etiqueta'] for item in etiq_def.values() if item['etiqueta'] != ""])
    todas_fases_defensa.columns = ['Fase']    

    cruce_fases_defensa= pd.merge(todas_fases_defensa, resumen_defensas, on ='Fase', how='left')
    cruce_fases_defensa.fillna(0,inplace=True)
    
    cruce_fases_defensa = cruce_fases_defensa.sort_values(by='Defensas', ascending=False)
    
     # Crear el gráfico combinado
    fig, ax1 = plt.subplots(figsize=(10, 6))

    # Gráfico de barras para los ataques
    bars = ax1.bar(cruce_fases_defensa['Fase'], cruce_fases_defensa['Defensas'], color='lightblue', label='Ataques', alpha=0.7)
    ax1.set_xlabel('Fase')
    ...
    # Crear un segundo eje para la línea del porcentaje de goles
    ax2 = ax1.twinx()
    ax2.plot(cruce_fases_defensa['Fase'], cruce_fases_defensa['% Exito'], color='green', marker='o', label='% Exito', linestyle='-', linewidth=2)
    ax2.set_ylabel('% Éxito', color='green')
    ax2.tick_params(axis='y', labelcolor='green')
    ax2.set_ylim(0, 100)

    # Añadir los valores sobre los puntos de la serie de líneas
    for i, (fase, pct, val) in enumerate(zip(cruce_fases_defensa['Fase'], cruce_fases_defensa['% Exito'] , cruce_fases_defensa['Defensas'])):
        if (pct != 0):
            ax2d.annotate(f'{pct:.1f}%', (i, pct), textcoords="offset points", xytext=(0, 10), ha='center', color='green')
